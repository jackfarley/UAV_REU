...
class uav_collab(ParallelEnv):
    ...
    def step(self, actions):
        ...
        rewards = {}
        for ind in range(len(self.agents)):
             rewards[self.agents[ind]] = -1 + -.3*(self.dist(self.U_t[ind], self.L_f[ind])) + self.check_fin_mild(self.R_G)  + cont_r[ind]#(-3 * self.c_U[ind]) #self.dist(self.U_t[ind], self.L_s[ind]) #(0 * self.c_U[ind])   
        self.ts += 1
        print(self.U_t)
