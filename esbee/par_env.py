...
class uav_collab(ParallelEnv):
    ...
    def reset(self, seed=0, options={"options": 1}):
        ...
        observations = {
            a:  self.U_t[self.agents.index(a)]  + (self.c_cont[self.agents.index(a)],)
    ...
    def step(self, actions):
        ...
        if cont_r[0] < 0:
             rewards = {a:  self.wall_pen for a in self.agents}
             
        else:
             rewards = {}
             for ind in range(len(self.agents)):
                rewards[self.agents[ind]] = d_dis[ind] - 1#+  self.speed_mat[self.global_episodes]#+ 4*cont_r[ind]  -1 # gbs_new[ind] #  pscp -r C:\Users\JackF\Documents\REU\uav_environment\env\ farleyj3@tesla.cs.vcu.edu:/home/farleyj3/rl -.1*(self.dist(self.U_t[ind], self.L_f[ind]))  (-3 * self.c_U[ind]) #self.dist(self.U_t[ind], self.L_s[ind]) #(0 * self.c_U[ind])   
