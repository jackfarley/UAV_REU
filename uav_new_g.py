...
class uav_collab(ParallelEnv):
    ...
    def reset(self, seed=0, options={"options": 1}):
        ...
        observations = {
            a: (self.c_cont[self.agents.index(a)],) + self.U_t[self.agents.index(a)] + self.L_f[self.agents.index(a)] + self.tu_form(self.g)
    ...
    def step(self, actions):
        ...
        if j:
             ...
             rewards = {a: 10000 for a in self.agents}
        ...
        rewards = {}
        for ind in range(len(self.agents)):
             rewards[self.agents[ind]] = -1 + -.3*(self.dist(self.U_t[ind], self.L_f[ind]))   + cont_r[ind] + self.check_fin_mild(self.R_G) #  (-3 * self.c_U[ind]) #self.dist(self.U_t[ind], self.L_s[ind]) #(0 * self.c_U[ind])   
        self.ts += 1
        print(self.U_t)
