...
class uav_collab(ParallelEnv):
    ...
    def reset(self, seed=0, options={"options": 1}):
        ...
        observations = {
            a:  self.U_t[self.agents.index(a)] + self.L_f[self.agents.index(a)] + self.tu_form(self.g)
    ...
    def step(self, actions):
        ...
        rewards = {}
        for ind in range(len(self.agents)):
             rewards[self.agents[ind]] = -1 + cont_r[ind] +  gbs_new[ind] + 10*d_dis[ind]  # pscp -r C:\Users\JackF\Documents\REU\uav_environment\env\ farleyj3@tesla.cs.vcu.edu:/home/farleyj3/rl -.1*(self.dist(self.U_t[ind], self.L_f[ind]))  (-3 * self.c_U[ind]) #self.dist(self.U_t[ind], self.L_s[ind]) #(0 * self.c_U[ind])   
        self.ts += 1
        print(self.U_t)
    ...
    def observation_space(self, agent):
        return MultiDiscrete([self.o_max] + [self.grid_size] * (4 + (len(self.g) * 2)))
