...
def parse_args():
    ...
    parser.add_argument("--g", type=list, default=[ (0,0), (5,5), (10,10), (15,5)],
                    help="list of gbs locations")
    parser.add_argument("--L_s", type=list, default=[(0, 0)],
    ...
    parser.add_argument("--total_timesteps", type=int, default=128,
...
if __name__ == "__main__":
    ...
    rb_obs = torch.zeros((args.max_cycles, num_agents * args.runs_per_batch, agent.obs_len)).to(device)
    rb_actions = torch.zeros((args.max_cycles, num_agents* args.runs_per_batch)).to(device)
    rb_logprobs = torch.zeros((args.max_cycles, num_agents* args.runs_per_batch)).to(device)
    rb_rewards = torch.zeros((args.max_cycles, num_agents* args.runs_per_batch)).to(device)
    rb_terms = torch.zeros((args.max_cycles, num_agents* args.runs_per_batch)).to(device)
    rb_values = torch.zeros((args.max_cycles, num_agents* args.runs_per_batch)).to(device)
